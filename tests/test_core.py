...
class SplitTests(unittest.TestCase):
    def test_overlap_and_metadata(self):
        pages = [{"source": "os.pdf", "page": 7, "text": "abcdefghij"}]
        chunks = split_text_by_page(pages, 6, 2)
        self.assertEqual(chunks, [
            {"source": "os.pdf", "page": 7, "text": "abcdef"},
            {"source": "os.pdf", "page": 7, "text": "efghij"},
        ])

    def test_short_and_blank_pages(self):
        self.assertEqual(split_text_by_page([{"page": 1, "text": "abc"}]), [{"source": "unknown.pdf", "page": 1, "text": "abc"}])
...
class SearchTests(unittest.TestCase):
    def setUp(self):
        self.docs = [
            {"source": "os.pdf", "page": 2, "text": "死锁的必要条件包括互斥、请求与保持、不可剥夺和循环等待"},
            {"source": "net.pdf", "page": 8, "text": "计算机网络分为物理层、数据链路层和网络层"},
        ]
        self.index = SearchIndex(self.docs)

    def test_chinese_question_finds_relevant_page(self):
        result = self.index.search("死锁的必要条件是什么")[0]
        self.assertEqual((result["source"], result["page"]), ("os.pdf", 2))
    ...
    def test_invalid_search_options(self):
        for count in [0, -1, 1.5, True]:
            with self.assertRaises(ValueError):
                self.index.search("死锁", count)
        for score in [-0.1, 1.1, True, "0.1"]:
            with self.assertRaises(ValueError):
                self.index.search("死锁", min_score=score)
...
class PdfTests(unittest.TestCase):
    def test_original_page_and_filename_survive_blank_page(self):
        with pymupdf.open() as document:
            document.new_page()
            page = document.new_page()
            page.insert_text((72, 72), "Operating systems and deadlock")
            data = document.tobytes()
        pages = parse_pdf_bytes(data, "../course.pdf")
        self.assertEqual(len(pages), 1)
        self.assertEqual((pages[0]["source"], pages[0]["page"]), ("course.pdf", 2))
