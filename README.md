...
安装并启动 Ollama 后，在页面中填写服务地址和本地模型名称。已使用 `qwen2.5:1.5b` 在 `http://127.0.0.1:11434` 完成真实调用验证。

> OpenAI兼容接口已通过本地模拟服务测试；Ollama本地模型已完成真实调用验证。真实课程问答质量仍取决于所选模型、资料和问题，需要使用人工标注的数据集继续评估。
...
验证结果见 [本地模型验证记录](docs/ollama-verification.md)。
